...
- FULL-UI-01-P1B: dense audit list now renders through the shared `@oneday/ui` `Table` (`od-table`), and risk/evidence detail surfaces through the shared `Modal` (`od-modal`); Playwright asserts `permission-audit-table` and `permission-audit-modal` test ids. Maps to `COMMERCIAL_ACCEPTANCE_MATRIX` UI-01 (design-system consistency) and UI-03 (feedback/confirm). Screenshot `management-permission-audit-desktop-v3-table-modal.png`.
