...
- [ ] **NEXT:** **W∞-53** 续 平台/商圈/员工 深度 densify（Management MPC 真实数据深页 W∞-45~51 + 平台面 `/p/agents` W∞-52 已收；续平台 `/p/tenants` `/p/channels` `/p/outbox` `/p/security-audit` 真实数据深页密度 或 商圈 `/bc/*` / 员工 `/e/*` 剩余真实数据面下一处缺口）。
