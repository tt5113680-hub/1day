...
- [ ] **NEXT:** **W∞-63** 续 平台/商圈/渠道/员工 深度 densify（Management MPC 真实数据深页 W∞-45~51 + 平台面 `/p/agents` W∞-52 + `/p/tenants` W∞-53 + `/p/channels` W∞-54 + `/p/business-circles` W∞-55 + `/p/outbox` W∞-56 + `/p/security-audit` W∞-57 + `/p/connectors` W∞-58 + `/p/templates` W∞-59 + 商圈面 `/bc/dashboard` W∞-60 + 平台面 `/p/dashboard` W∞-61 + 渠道面 `/ch/dashboard` W∞-62 已收；续商圈 `/bc/merchants` / 渠道 `/ch/*` 剩余 / 员工 `/e/*` 剩余真实数据面下一处缺口）。
