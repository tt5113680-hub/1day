...
- [x] SYS-6 channel/circle network packs PASS — channel/circle dashboards + merchant writes filter/deny by `data_scopes`; `platform.manage` unrestricted; circle dashboard accepts `circle.manage`; see `SYS_6_ACCEPTANCE.md`. Store Manager content writes / remaining write controllers remain.
