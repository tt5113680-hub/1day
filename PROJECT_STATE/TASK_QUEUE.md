...
- [ ] HARDENING-006 IN PROGRESS - Docker workspace dependency build order is being repaired for API, Worker and human-pilot image targets. Server clean-image validation exposed required `@oneday/session-client` and `@oneday/ui` builds; a fresh image verification is pending. Evidence: `evidence/HARDENING-006/ACCEPTANCE.md`.
