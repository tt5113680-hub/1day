...
- [x] SYS-6 write-path scopes PASS — redeem gated by enrollment store scope; assigned store managers may list/update store commercial + external links; assign-manager remains owner-only; see `SYS_6_ACCEPTANCE.md`. More write controllers / channel-circle packs remain.
