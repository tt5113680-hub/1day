...
- [ ] **NEXT:** **W∞-76** 消费者剩余 PARTIAL densify（`/c/profile` 已收、`/c/circles` 已收；续 `circle-detail` 商圈详情 `/c/circles/[id]`、搜索 `/c/search`、外链确认 `/c/actions/[id]` 等剩余 PARTIAL）或平台/员工剩余 PARTIAL densify。
