...
| MPC-11 | 店铺装修 / 展示 | `/m/page-builder`, `/m/content` | PARTIAL | **壳跟美团**；内容数据仍走 ONEDAY 发布链；W∞-25 眉标 `入口页装修`/`营销内容`（对齐导航）；**W∞-43** 视觉/IA densify（`/m/page-builder` + `/m/content` 黄顶栏+灰底白卡+heroCard+概况条+白卡面板）；**W∞-90** summaryStrip 收敛全标对黄线浅黄底概况条（`/m/content` 去退化白卡变体）；toward PARITY；见 WINF43/WINF90 |
...
| ME-03 | 顾客 | `/e/customers`, `/e/nurture` | PARTIAL | W∞-16 工具身份；**W∞-65** 真实数据深页分布（归属/状态/待办负载/建档窗口，source=local，禁止假 BI）；**W∞-95** 客户跟进队列 `/e/nurture` full-parity（topBar+heroCard+summaryStrip+distribution+honest，真实 profiles[] 推导，禁止假 BI）；toward PARITY；见 WINF65/WINF95 |
