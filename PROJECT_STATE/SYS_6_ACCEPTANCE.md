# SYS-6 Role IA / multi-product + data_scopes + write-path + network packs + content placements

## Result

`SYS_6_MULTI_PRODUCT_PASS` + `SYS_6_DATA_SCOPES_PASS` + `SYS_6_WRITE_PATH_SCOPES_PASS` + `SYS_6_NETWORK_PACKS_PASS` + `SYS_6_CONTENT_PLACEMENTS_PASS` (local engineering). Not full Role matrix E2E across every controller. Not 全部商用. Not Tencent Cloud.
...
| API menu           | `GET /api/v1/me/menu?product=*` + Store Manager 门店/内容 + channel/circle scope context labels      |
