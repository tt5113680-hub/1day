...
## 2. 大厂对标原则（美团 100% UI + 可换肤 token · 2026-08-10）

**权威细则：** `PROJECT_STATE/PRODUCT_DUAL_TRACK_STRATEGY.md`

| 项 | 裁决 |
| -- | ---- |
| PC 端 + H5 | **100% 一模一样复制美团**（视觉/IA/交互/功能链路/页面结构） |
| 唯一定制 | **仅工作流整合页**单独定制；废止「首页内容 DIY」宽口径 |
| 默认色 | **美团黄**（`--od-brand-*`）；**不锁企微蓝** |
| 换肤 | 只改 `packages/design-tokens`（含可选 `themes.css` / `data-theme`） |
| 渠道 | 美团各级代理 |
| 开发节奏 | **宁快**：按美团页同构落地，少纠结色板 |

**不做进第一阶段：** 九角色全包、任意 HTML 低代码、假 BI、宣称已接美团实时库存。  
**本阶段必须纠正：** 平台/渠道扁平向导 → 省市区代理树开通。
