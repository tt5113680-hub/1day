...
- next_scope: Product owner visual acceptance for `CONSUMER-COMMERCIAL-HOME-V1`. The Consumer storefront is available on the isolated local HUMAN-PILOT database with persisted package-platform prices (migration 047), an LBS/future-recommendation header, a verified store-information card layout, and a product-detail page that renders package information, purchase notes and store-scoped platform offers; do not expand Employee, Management or Platform while this acceptance is pending.
