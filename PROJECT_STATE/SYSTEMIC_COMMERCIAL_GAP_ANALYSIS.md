...
- Employee menu DTO + **SYS-27** desktop nav ≥700px + store-manager mode/package on `/e/store` + **SYS-30** `/e/tasks` inbox + **SYS-31** `/e/customers` directory (full nine-role packages still multi-week)
...
| Employee   | ~7.5/10    | Desktop nav + task inbox + customer directory (SYS-27/30/31)         |
...
2. Member enroll → wallet → Employee redeem (**SYS-33** `/e/memberships`) → Management grant/revoke timeline (**SYS-34** ledger + revoke)
