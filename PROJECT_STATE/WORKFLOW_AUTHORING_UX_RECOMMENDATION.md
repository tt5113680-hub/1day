...
- baseline: SYS-10…SYS-16 PASS (`00ee969`)
...
| **SYS-17 (next eng)** | **Insert rails** | Between-step「在此插入」+ duplicate step; still ordered list |
| **SYS-18** | **Condition card IA** | Dedicated card UI: when-true continues / when-false skips next; copy explains API limits |
| **SYS-19** | **Start-context presets** | Named sample contexts (e.g. `upsell=true`) saved locally for simulator |
