...
- last_verified: **2026-08-12** — W∞-74 消费者门店页 `/c/stores/[id]`（MH5-03，商家消费者页）真实数据深页 densify（白卡 heroCard+summaryStrip 门店数据概况+门店入口分布面板：平台入口/服务类型/行动入口/内容类型/装修模块五组，全部由真实 StoreDetail 行推导，禁止假 BI）PASS。员工 ME-02~07 + 渠道开通 + 消费者频道深页（W∞-71）+ 消费者统一入口（W∞-72）+ 消费者发现/附近（W∞-73）+ 消费者门店页（W∞-74）已收。See WINF74.
...
- progress: Next: **W∞-75** 消费者剩余 PARTIAL densify（`/c/profile` 我的 MH5-09、`/c/circles` 商圈联盟 MH5-13 等）或平台剩余 PARTIAL densify。
