...
export class ManagementCustomerAssetsService implements OnModuleDestroy {
  ...
  async list(context: OrganizationContext, query: Record<string, unknown>) {
    const search = text(query.search, 160),
      ...
      layer = text(query.layer, 40);
    if (owner && !uuid.test(owner)) throw new BadRequestException('VALIDATION_ERROR');
    const layerWhere = layer
      ? ` and exists(select 1 from customer_rfm_profiles r2 where r2.tenant_id=c.tenant_id and r2.customer_id=c.id and r2.deleted_at is null and r2.layer=$7)`
      : '';
    const rows = await this.pool.query(
      `select c.id,c.display_name,c.version,coalesce(o.employee_id::text,'') owner_id,coalesce(u.display_name,'未分配') owner_name,
       ...
       r.recency_days,r.frequency_count,r.reach_count,r.layer
       ...
       left join customer_rfm_profiles r on r.tenant_id=c.tenant_id and r.customer_id=c.id and r.deleted_at is null
       ...
       ${layerWhere}
       group by c.id,c.display_name,c.version,o.employee_id,u.display_name,n.segment,r.recency_days,r.frequency_count,r.reach_count,r.layer order by c.updated_at desc limit 200`,
      layer
        ? [context.tenantId, search, tag, segment, owner, source, layer]
    ...
    return rows.rows.map((row) => ({
      ...
      rfm: {
        ...
        recencyDays: row.recency_days ?? null,
        frequencyCount: row.frequency_count ?? null,
  ...
  async detail(context: OrganizationContext, id: string) {
    ...
    const [
      ...
      this.pool.query(
        `select f.action_type,f.summary,f.created_at,coalesce(eu.display_name,'Unknown employee') employee_name
           from task_follow_ups f join tasks tk on tk.id=f.task_id
           left join employees fe on fe.id=f.employee_id and fe.tenant_id=f.tenant_id
           left join memberships fm on fm.id=fe.membership_id and fm.tenant_id=fe.tenant_id left join users eu on eu.id=fm.user_id
           where tk.tenant_id=$1 and tk.customer_id=$2 and f.deleted_at is null and tk.deleted_at is null
    ...
    const timeline = [
      ...
      ...followUps.rows.map((item) => ({
        kind: 'follow_up',
        label: `${item.action_type}: ${item.summary ?? '跟进'}`,
        at: item.created_at,
      })),
    ]
      .sort((left, right) => String(right.at).localeCompare(String(left.at)))
      .slice(0, 60);
    const rfmProfile = rfm.rows[0] ?? null;
    return {
      customer: {
        id: detail.id,
      ...
      rfm: rfmProfile
        ? {
            recencyDays: rfmProfile.recency_days,
            frequencyCount: rfmProfile.frequency_count,
            reachCount: rfmProfile.reach_count,
            layer: rfmProfile.layer,
            windowDays: rfmProfile.window_days,
            computedAt: rfmProfile.computed_at,
          }
        : null,
  ...
  async downloadExport(context: OrganizationContext, id: string, requestId: string) {
    ...
    const csv = [
      'customer_id,display_name,segment,rfm_layer,owner,tags,active_orders',
