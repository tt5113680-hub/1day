...
test('employee generates an actual QR share code at 390px and can invalidate it', async ({
  ...
  await page.screenshot({ path: 'evidence/PAGE-E-005/employee-share-mobile.png', fullPage: true });
...
test('employee share route shows recovery without a session', async ({ page }) => {
  await page.goto('/e/share');
  await expect(page.getByRole('heading', { name: '无法管理分享码' })).toBeVisible();
  await page.screenshot({
    path: 'evidence/PAGE-E-005/employee-share-forbidden.png',
    fullPage: true,
  });
});
